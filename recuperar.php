...
// 1. IMPORTAÇÃO DOS CONECTORES CENTRALIZADOS SÉCURA
require_once __DIR__ . "/config/Banco.php";
include_once __DIR__ . "/Conexao.php";

// Garante o reaproveitamento do link de conexão ativo
\$mysqli = \(conexao_link ?? \)conexao_aurelius ?? conexao ?? mysqli ?? null;

if (!mysqli || !(mysqli instanceof mysqli)) {
    die("<div style='padding:20px; background:#0f172a; color:#ef4444; font-family:sans-serif; text-align:center;'>
            <strong>Erro de Infraestrutura:</strong> O conector central MySQLi não foi localizado na raiz do servidor.
         </div>");
}
...
if (isset(\$_POST['recuperar'])) {
    ...
    if (empty(\(email)) {\)erro = "Por favor, digite o seu e-mail cadastrado.";
    } else {
        ...
        if (\$check_user && mysqli_num_rows(\(check_user) > 0) {\)user_data = mysqli_fetch_assoc(\(check_user);\)id_cliente = \(user_data['codigo'];\)nome_cliente = \(user_data['name'] ?? \)user_data['nome'];
            ...
            // Atualiza a tabela do cliente com a nova credencial limpa
...
<div class="container">
    ...
    <!-- Força a submissão estrita para o próprio arquivo correto evitado o 404 -->
    ...
    <!-- Corrigido para apontar para o arquivo correto de entrada do seu sistema local -->
    <a href="Principal.php" class="btn-voltar">← Voltar ao Menu Principal</a>
</form>
</div>
