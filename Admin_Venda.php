...
// Agora sim, importa o banco com segurança uma única vez
require_once __DIR__ . "/config/Banco.php";

// 1. MOTOR REATIVO DE MARCAÇÃO DE LEITURA (SINO/NOTIFICAÇÕES)
...
// 🔐 3. FILTRAGEM MULTI-INQUILINO & CAPTURA DO NOME DA EMPRESA
...
// 🗂️ 4. CAPTURA AUTOMÁTICA DO IBAN DO GRUPO AURÉLIUS (DA TABELA CONFIGURACOES)
...
// 🔍 5. CONSULTA EXCLUSIVA PARA A ABA MASTER SAAS (DADOS FINANCEIROS GLOBAIS COM INNER JOIN)
...
// 🟢 6. MOTOR DE PROCESSAMENTO FINANCEIRO SAAS
...
// 💸 7. MOTOR DE LIQUIDAÇÃO CENTRAL SAAS (REPASSE MANUAL)
...
<body>
	...
	<div class="SaaS-vendas-container">
		...
		<div class="topo-vendas-SaaS" style="display: flex; justify-content: space-between; align-items: center;">
			<div style="text-align: left;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
					...
					<span style="background: rgba(234, 179, 8, 0.1); border: 1px solid #eab308; color: #eab308; font-size: 10px; padding: 2px 8px; border-radius: 4px; font-weight: bold;">
						<?= htmlspecialchars($nome_parceiro_ativo) ?>
					</span>
				</div>
		...
		<div class="menu-abas">
			<button class="aba-btn ativa" onclick="mudarAbaInterna(1)"> As Minhas Vendas</button>
			<button class="aba-btn" onclick="mudarAbaInterna(2)" style="border-color: #ca8a04; color: #eab308;"> Mesa de Controlo SaaS </button>
		</div>
			...
			<div class="grid-vendas-cards">
				...
				<div class="card-pedido-SaaS">
					<div style="border-bottom: 1px solid #1f2937; padding-bottom: 10px; margin-bottom: 15px;">
						<strong style="color: #38bdf8; font-size: 15px; display: block;">
						...
						<span style="color: #64748b; font-size: 11px;"> Data:
					...
					<div style="font-size: 13px; color: #cbd5e1; line-height: 1.6; margin-bottom: 15px;">
						<p style="margin: 4px 0;">
							<b>Empresa:</b>
							<?= htmlspecialchars($pedido['loja']) ?>
						</p>
						<p style="margin: 4px 0;">
							<b>Sede:</b>
							<?= htmlspecialchars($pedido['local']) ?>
						</p>
						<p style="margin: 4px 0;">
							<b>Contacto:</b>
						...
						<p style="margin: 4px 0;">
							...
							<span style="color: #a855f7; font-weight: bold;">
								<?= $is_domicilio ? 'Domiciliário' : 'Balcão' ?>
							</span>
						</p>
					</div>
					...
					<div>
						...
						<?php endif; ?>
						<?php else: ?>
						<button type="button" class="btn-SaaS-acao" style="background: #1e293b; color: #64748b; width: 100%; border: 1px solid #334155; cursor: not-allowed;"
						 disabled> Enviado ao SaaS Central</button>
						<?php endif; ?>
					</div>
				</div>
				<?php endforeach; ?>
		...
		<div id="aba-interna-2" class="aba-painel">
			<div style="background: rgba(202, 138, 4, 0.05); border: 1px dashed #ca8a04; padding: 20px; border-radius: 12px; margin-bottom: 25px;">
				<span style="color: #eab308; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px;"> CONTA DE CUSTÓDIA CENTRAL :</span>
				<p style="margin: 8px 0 0 0; font-family: monospace; font-size: 16px; color: #fff; font-weight: bold; letter-spacing: 1px;">
				IBAN ARRECADAÇÃO:
					<?= htmlspecialchars($iban_grupo_aurelius) ?>
				</p>
			</div>
			...
			<table class="tabela-financeira">
				...
				<tbody>
					...
					<tr>
						<td colspan="8" style="text-align: center; color: #64748b; padding: 40px; font-style: italic;">Nenhum repasse mercantil aguardando liquidação neste ciclo.</td>
					</tr>
					<?php else: ?>
					...
					<tr>
						<td>
							<strong>
								<?= htmlspecialchars($row['nome_loja']) ?>
							</strong>
						</td>
						<td>
							<?= htmlspecialchars($row['nome_cliente']) ?>
							<br>
							<small style="color:#a855f7; font-weight: bold;">
								<?= htmlspecialchars($row['tipo_atendimento']) ?>
							</small>
						</td>
						<td style="color: <?= $row['confirmado_na_entrega'] == 1 ? '#22c55e' : '#ef4444' ?>; font-weight: bold;">
							<?= $row['tipo_atendimento'] !== 'Domicilio' ? 'Balcão' : ($row['confirmado_na_entrega'] == 1 ? '✓ Sim' : '⏳ Não') ?>
						</td>
						<td style="font-weight: bold; color: #fff;">
							<?= number_format($row['valor_bruto'], 2, ',', '.') ?>Kz</td>
						<td style="color: #ef4444; font-weight: bold;">-
							<?= number_format($row['comissao_retida'], 2, ',', '.') ?>Kz</td>
						<td style="color: #22c55e; font-weight: bold; font-size: 14px;">
							<?= number_format($row['valor_liquido'], 2, ',', '.') ?>Kz</td>
						<td style="color: #eab308; font-family: monospace; font-weight: bold;">
							<?= htmlspecialchars($row['iban_loja']) ?>
						</td>
						<td>
							<form method="POST" onsubmit="return confirm('Autorizar transferência de <?= number_format($row['valor_liquido'], 2, ',', '.') ?> Kz para o IBAN cadastrado?');"
							 style="margin:0;">
