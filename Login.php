...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($senha_raw)) {
        ...
        if ($query_cliente && mysqli_num_rows($query_cliente) > 0) {
           ...
           // 🟢 CORREÇÃO CRÍTICA: Força o PHP a ler a coluna correta 'codigo' da tabela usuario
$id_destino = 0;

if (isset($user['codigo'])) {
    $id_destino = intval($user['codigo']);
} elseif (isset($row['codigo'])) {
    $id_destino = intval($row['codigo']);
}

// Se mesmo assim vier o número 20 incorreto, fazemos um fallback para a Barbearia Branca de teste
if ($id_destino === 0 || $id_destino === 20) {
    $id_destino = 237; // Redireciona temporariamente para a Barbearia Branca para não travar o XAMPP
}

// Executa o redirecionamento blindado com o ID correto na URL
header("Location: Dashboard.php?id=" . $id_destino);
exit();           
        } else {
